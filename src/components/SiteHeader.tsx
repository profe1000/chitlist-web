    ...
    <>
      ...
      <header className="sticky top-0 z-40 border-b border-black/5 bg-[#fffaf2]/95 backdrop-blur">
        <div className="page-shell flex h-19.5 items-center justify-between">
          <Brand />
          <nav className="hidden items-center gap-8 text-sm font-bold lg:flex" aria-label="Main navigation"><Link className={`nav-link ${isHome ? 'active' : ''}`} to="/#home">Home</Link><Link className="nav-link flex items-center gap-1" to="/#services">Our Services <ChevronDown size={14} /></Link><Link className="nav-link" to="/#story">About Us</Link><Link className="nav-link" to="/#locations">Our Locations</Link><Link className="nav-link" to="/#impact">Sustainability</Link><Link className={`nav-link ${!isHome ? 'active' : ''}`} to="/contact">{isHome ? 'Contact' : 'Contact Us'}</Link></nav>
